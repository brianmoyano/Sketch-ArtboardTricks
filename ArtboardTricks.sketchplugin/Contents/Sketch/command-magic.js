...
function numbersAdd(context) {
  ...
  for (var _i = 0; _i < artboardMetas.length; _i++) {
    ...
    var prefix = util.zeropad(row, 2) + currentPrefs.rowColSeparator + util.zeropad(col, 2) + (subCol > 0 ? '.' + subCol : '') + (baseName ? currentPrefs.numberTitleSeparator : '');

    // add prefix to the name
    meta.artboard.setName('' + String(currentNamePath) + prefix + String(baseName));
  }
}
